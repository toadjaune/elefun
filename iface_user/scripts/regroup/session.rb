module Regroup
  module Sessions
    ...
    def self.global_activity(min, max)
      #À DÉFINIR
      # Je propose de regarder par rapport au nombre d'activité par heure
      if min == -1
        #plus de
        return Session.as(:s).when("3600 * ( s.quiz_answered + s.video_viewed + s.forum_posted ) >= #{max}*#{s.end-s.start}").count
      elsif max == -1
        #moins de
        return Session.as(:s).when("3600 * ( s.quiz_answered + s.video_viewed + s.forum_posted ) <= #{min}*#{s.end-s.start}").count
      else
        #range
        return Session.as(:s).when("3600 * ( s.quiz_answered + s.video_viewed + s.forum_posted ) >= #{max}*#{s.end-s.start} and 3600 * ( s.quiz_answered + s.video_viewed + s.forum_posted ) <= #{min}*#{s.end-s.start}").count
      end
    end
  end
end
